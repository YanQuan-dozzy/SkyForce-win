...
SkyForce 是一款 Windows 桌面乐谱工作台，专注于 SkyStudio 乐谱的转换、编辑与 AutoJS 脚本生成。它是 Electron 单机应用，可在 Windows 10/11 64 位系统上独立运行，**无需安装 phpStudy、PHP、MySQL、Node.js 或浏览器**。乐谱转换、编辑、MIDI 提取与制图全部在本机完成，只有可选的 DeepSeek AI 和弦才需要联网。
...
| 资料导入 | 从本地目录导入既有模板、转换结果与音色资源，源目录只读 |
...
- **历史记录**：转换记录、状态与耗时
- **设置与数据**：主题、21 键开关、默认导出目录、DeepSeek 配置、资料导入、自定义脚本模板

## 技术架构

### 技术选型

- **Electron**：自带 Chromium 与 Node.js 运行时，成品无需用户安装浏览器、PHP 或 Node.js
- **TypeScript**：全栈类型安全
- **Vite**：渲染进程构建
- **`node:sqlite`**：随 Electron 运行时提供的本地数据库
...
- [`src/electron/main.ts`](src/electron/main.ts)：窗口、协议、IPC、文件、批处理、日志、DPAPI 与资料导入
...
- 简谱图片采用本地 Canvas 排版，不复刻网页的全部字体与主题
- 自定义音色包目前通过本地资料导入
